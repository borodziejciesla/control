...
class Pid {
 ...
 private:
  ...
  double RateLimitControl(const double control) const;
  ...
  double min_control_ = std::numeric_limits<double>::infinity();
  double max_control_ = -std::numeric_limits<double>::infinity();
  double rate_limit_ = std::numeric_limits<double>::infinity();
